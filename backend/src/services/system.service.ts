...
export async function getBootstrap() {
  const [
    ...
    purchaseOrders, employees, attendanceRecords, payrollRecords, customers, configRows,
  ] = await Promise.all([
    ...
    prisma.customer.findMany(), prisma.appConfig.findMany(),
  ]);
  ...
  return {
    items, branchStocks, combos, stockAdjustmentLogs, estimates, challans, invoices,
    enquiries, pendingOrders, reminders, cashRegisters, recurringExpenses, vendors,
    purchaseOrders, employees, attendanceRecords, payrollRecords, customers, ...config,
  };
}
