...
export const InventorySettingsModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white border border-slate-300 rounded-none w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh] flex flex-col text-slate-900">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-300 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 uppercase tracking-wide">Inventory Settings</h2>
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] font-mono font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded-none border border-red-200">
                {thresholdDays} Days
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-1.5 mb-3">
              {PRESET_DAYS.map((days) => (
                <button
                  ...
                  className={cn(
                    'py-1.5 text-xs font-bold rounded-none border transition-all cursor-pointer',
                    thresholdDays === days
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3.5 py-2 rounded-none bg-white border border-slate-300 text-slate-900 font-bold text-sm focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 pr-16"
                required
              />
          ...
          <div className="p-3.5 rounded-none bg-amber-50/70 border border-amber-200 text-[11px] text-amber-800 space-y-1">
          ...
          <div className="p-3.5 rounded-none bg-slate-50 border border-slate-300 text-[11px] text-slate-600 flex items-start gap-2">
          ...
          {/* Footer Actions */}
          <div className="pt-2 flex items-center justify-end gap-2.5 border-t border-slate-300">
            <button
              type="button"
              onClick={onClose}
